...
export default function Sidebar({ isOpen, onClose, navigation, currentRoute, user }) {
  ...
  const handleNavigation = (route) => {
    // Tab routes are nested inside MainTabs
    const tabRoutes = ['Home', 'Browse', 'Upload', 'Profile'];
    if (tabRoutes.includes(route)) {
      // Navigate to MainTabs with screen param for nested navigator
            ...
            <View style={styles.userSection}>
              <View style={styles.userAvatar}>
                <Text style={styles.userInitials}>
                  {(user.name || user.fullName) ? (user.name || user.fullName).split(' ').map(n => n[0]).join('').toUpperCase() : 'U'}
                </Text>
              </View>
          ...
          <View style={styles.menuSection}>
            ...
            {menuItems.map((item, index) => (
              ...
              >
                <item.icon 
                  size={22} 
                  color={currentRoute === item.route ? '#2d8f3e' : '#666'} 
                />
                <Text style={[
                  styles.menuItemText, 
          ...
          <View style={styles.menuSection}>
            {secondaryItems.map((item, index) => (
              ...
              >
                <item.icon 
                  size={20} 
                  color={currentRoute === item.route ? '#2d8f3e' : '#888'} 
                />
                <Text style={[
                  styles.menuItemTextSecondary,
                  currentRoute === item.route && styles.menuItemTextActive
                ]}>
                  {item.label}
                </Text>
